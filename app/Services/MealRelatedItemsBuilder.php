...
class MealRelatedItemsBuilder
{
    public static function build($paginator, $lang, $with)
    {
        ...
        $paginator->getCollection()->transform(function ($meal) use ($langId, $with) {
            ...
            if (in_array('tags', $with)) {
                $result['tags'] = $meal->tags->map(function ($tag) use ($langId) {
            ...
            if (in_array('ingredients', $with)) {
                $result['ingredients'] = $meal->ingredients->map(function ($ingredient) use ($langId) {
